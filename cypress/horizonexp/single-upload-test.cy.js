describe('HorizonExp Single Upload Test Suite', () => {
  ...
  it('Should successfully upload a file through the complete user journey', () => {
    ...
    cy.wait(2000);
    cy.contains('label, span', 'Channel', { matchCase: false }).first().parents('div').find('button, [role="button"]')
      .filter(':visible')
      .first()
      .should('contain', `DevOps' Channel`);
    cy.contains('label, span', 'Category', { matchCase: false }).first().parents('div').find('button, [role="button"]')
      .filter(':visible')
      .first()
    ...
    cy.get('input[placeholder*="title"], input[name="title"]')
      ...
      .first()
      .clear({ force: true })
      .type('Test Upload Video', { force: true });
    ...
    cy.get('textarea[placeholder*="caption"], textarea[name="caption"], input[placeholder*="caption"]')
      ...
      .first()
      .clear({ force: true })
      .type('This is a test caption generated by automation.', { force: true });
    ...
    cy.get('input[placeholder*="tag"], input[name="tags"]')
      ...
      .first()
      .clear({ force: true })
      .type('automation{enter}test{enter}video{enter}', { force: true });
